  ...
  return (
    <div style={{ position: 'relative' }}>
      <pre className={className}>
        <code {...props.attributes} className={className} >{props.children}</code>
      </pre>
      <div
...
class CodeHighlighting extends React.Component {
  ...
  renderDecoration = (props, editor, next) => {
    ...
    // prism 主题, 添加 prism 高亮类名
    const attributesWithClassName = { ...attributes, className: `token ${decoration.type}` }
    switch (decoration.type) {
      case 'comment':
        return (
          <span {...attributesWithClassName} style={{ opacity: '0.33' }}>
            {children}
          </span>
        )
      case 'keyword':
        return (
          <span {...attributesWithClassName} style={{ fontWeight: 'bold' }}>
            {children}
          </span>
        )
      case 'tag':
      ...
      case 'punctuation':
        return (
          <span {...attributesWithClassName} style={{ opacity: '0.75' }}>
            {children}
          </span>
        )
      default:
